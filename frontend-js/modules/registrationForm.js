...
export default class RegistrationForm {
    ...
    passwordAfterDelay() {
        if (this.password.value.length < 12) {
            this.showValidationError(this.password, "Password must be more than 12 characters")
        }
    }
    ...
    usernameImmediately() {
        if (this.username.value != "" && !/^([a-zA-Z0-9])+$/.test(this.username.value)) {
            this.showValidationError(this.username, "username can only conain letters and numbers")
        }
